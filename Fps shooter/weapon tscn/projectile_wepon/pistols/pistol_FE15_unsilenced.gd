...
const smgv1_bullet=preload("res://bullets/lmg_bullets/silenced_lmg_bullets/carpanter150_silenced_bullet/carpanter150_silenced_bullet.tscn")
...
export var ads_acceleration:float=10.0
export var reload_rate:float=2
var current_ammo:int=0
var reloading:bool=false
var is_aiming_down_sight:bool=false
const rotation_y:int=-180
const rotation_x:int=90
const default_z_rotation:int=0


# Called when the node enters the scene tree for the first time.
func _ready():
	rotation.y=deg2rad(rotation_y)
	rotation.x=deg2rad(rotation_x)
	rotation.z=deg2rad(default_z_rotation)
	#set_process(false)
	#if parent is Node:
		#set_process(true)
...
func _process(_delta):
	if parent is Spatial:
		...
		if Input.is_action_pressed("aim_down_sight"):
			...
			transform.origin=transform.origin.linear_interpolate(aim_down_sight_position,ads_acceleration*_delta)
			camera.fov=lerp(camera.fov,camera_ads_fov,ads_acceleration*_delta)
			is_aiming_down_sight=true
		else:
			#is_aiming_down_sight=true
			transform.origin=transform.origin.linear_interpolate(defult_position,ads_acceleration*_delta)
			camera.fov=lerp(camera.fov,camera_defult_fov,ads_acceleration*_delta)
			is_aiming_down_sight=false
	else:
		set_process(false)
